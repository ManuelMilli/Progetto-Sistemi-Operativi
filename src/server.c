...
int reqQueueId, replyQueueId, maxProcSemId, maxProcMemId;

//gestione del C-c (chiusura del server)
void sig_handler(int sig) {
    if (sig == SIGINT) {
        printf("Disconnecting now...\n");
        msgctl(reqQueueId, IPC_RMID, NULL); //elimino la coda di messaggi delle richieste
        msgctl(replyQueueId, IPC_RMID, NULL); //elimino la coda di messaggi delle risposte
...
void setupIPC() {
    reqQueueId = msgget(COMMON_IPC_KEY, IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR); //crea la coda per le richieste
    if(reqQueueId == -1)
        errExit("impossibile creare la coda di messaggi per le richieste");
    replyQueueId = msgget(COMMON_IPC_KEY + 1, IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR); //crea la coda per le risposte
    if(replyQueueId == -1)
        errExit("impossibile creare la coda di messaggi per le risposte");
...
int main() {
    ...
    while(1){
        #ifdef DEBUG
        printf("[DEBUG] Polling in attesa...\n");
        char send;
        scanf("%c", &send);
        #endif

        if (msgrcv(reqQueueId, &imsg, mSize, LONG_MIN, 0) == -1) //ricevo il file più piccolo dalla coda di messaggi
    ...
    int memid = shmget(imsg.memory_key, imsg.mtype, 0); //mi collego alla memoria condivisa creata dal client
    if(memid == -1)
    ...
    rmsg.mtype = imsg.request_id; //imposto il tipo del messaggio di risposta con l'id della richiesta
    sha256(filebuf, imsg.mtype, rmsg.hash); //faccio l'hashing del contenuto del file che troverò in hash
    ...
    if(msgsnd(replyQueueId, &rmsg, mSize, 0) == -1) //invio del messaggio al client
        errExit("Errore nell'invio del messaggio di risposta");
